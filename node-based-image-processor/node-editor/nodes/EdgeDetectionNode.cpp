...
void EdgeDetectionNode::Process()
{
    ...
    cv::Mat outputImage = ApplyEdgeDetection(m_InputImage);
    
    // Set the output image in the ImageDataManager
    if (!outputImage.empty() && !Outputs.empty())
    {
        ImageDataManager::GetInstance().SetImageData(Outputs[0].ID, outputImage);
    }
...
void EdgeDetectionNode::DrawNodeContent()
{
    ...
    if (!outputImage.empty() && m_PreviewTexture)
    {
        ImGui::Separator();
        ImGui::Text("Preview:");
        ...
        float aspectRatio = (float)outputImage.cols / (float)outputImage.rows;
        float previewWidth = std::min(maxPreviewWidth, (float)outputImage.cols);
